...
>(({ className, children, hideCloseButton, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay className="md:data-[state=closed]:animate-none md:data-[state=open]:animate-none" />
    ...
    >
      {children}
      {!hideCloseButton && (
        <DialogPrimitive.Close className="absolute right-4 top-4 md:absolute rounded-full p-2 md:p-1 bg-muted md:bg-transparent hover:bg-accent md:hover:bg-transparent text-muted-foreground hover:text-foreground transition-colors touch-manipulation z-10">
...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex flex-col space-y-2 px-4 pt-4 md:px-6 md:pt-0 text-left", className)} {...props} />
);
DialogHeader.displayName = "DialogHeader";

const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex flex-col-reverse gap-2 px-4 pb-4 md:px-6 md:pb-0 md:flex-row md:justify-end md:space-x-2 mt-auto", className)} {...props} />
);
